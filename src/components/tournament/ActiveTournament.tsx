...
const formatDateTime = (partido: PartidoResuelto, includeTime = true) => {
  ...
  return partido.hora && includeTime
    ? `${formattedDate} · ${partido.hora}`
    : formattedDate;
};
...
function MatchCard({
  ...
  return (
    <article className="border-border bg-background rounded-xl border p-4 transition-shadow hover:shadow-md">
      <div className="mb-4 flex items-center justify-between gap-3 text-xs font-semibold tracking-wide uppercase">
        <span className="text-accent">
          {formatDateTime(partido, !estimated)}
        </span>
        <span
          className={
            hasScore
              ? "bg-accent/15 text-accent rounded-full px-2.5 py-1"
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-5 gap-y-3 text-sm sm:text-base">
        <span className={aWon ? "font-bold" : "font-medium"}>
          {participantName(partido.a)}
        </span>
        <span
          className={
            aWon
              ? "text-accent text-right font-bold tabular-nums"
              : "text-right tabular-nums"
          }
        >
          {scoreFor(partido, 0) || "—"}
        </span>
        <span className={bWon ? "font-bold" : "font-medium"}>
        ...
        <tbody>
          {rows.map(row => (
            <tr
              ...
              className={
                row.posicion === 1
                  ? "border-border bg-accent/5 border-t"
                  : "border-border border-t"
              }
            >
              <td className="text-accent px-3 py-3 font-bold">
                {row.posicion}°
              </td>
...
function Category({
  ...
  const playoffMatches = partidos.filter(
    partido => partido.fase !== "grupo" && visible(partido)
  );
  const zones = Array.from(new Set(parejas.map(pareja => pareja.zona ?? "")));

  return (
    <section
      className="border-border bg-muted/25 mt-10 rounded-2xl border p-4 sm:p-6"
      aria-labelledby={`category-${categoria}`}
    >
      <div className="border-border flex items-end justify-between gap-4 border-b pb-4">
        ...
        <span className="border-border bg-background rounded-full border px-3 py-1 text-xs font-semibold">
      ...
      <div
        className={`mt-8 grid gap-8 lg:grid-cols-2 ${playoffMatches.length > 0 ? "border-border border-t pt-6" : ""}`}
      >
        {zones.map(zone => {
          const zonePairs = parejas.filter(
            pareja => (pareja.zona ?? "") === zone
          );
          const zoneMatches = groupMatches.filter(
            partido => (partido.zona ?? "") === zone
          );
            ...
            <div key={zone || "unique"} className="min-w-0 pt-2">
              <div className="flex items-center justify-between gap-4">
                <h3 className="text-lg font-bold">
                  {zone ? `Zona ${zone}` : "Zona única"}
                </h3>
                <span className="text-foreground/65 text-xs">Posiciones</span>
              </div>
              <div className="mt-3">
...
export default function ActiveTournament({
  ...
  const matchesForSearch = useMemo(() => {
    ...
    return tournament.partidos
      ...
      .filter(partido =>
        normalizeSearch(
          `${participantName(partido.a)} ${participantName(partido.b)}`
        ).includes(query)
      )
      .sort(sortByDate);
  ...
  if (state.status === "loading")
    return (
      <p className="border-border rounded-lg border p-5">Cargando torneo…</p>
    );
  ...
  const categories = Array.from(
    new Set([
      ...state.tournament.parejas.map(pareja => pareja.categoria),
      ...state.tournament.partidos.map(partido => partido.categoria),
    ])
  );
  ...
  return (
    <>
      ...
      {showSearch && (
        <section
          className="border-border bg-background mt-8 rounded-2xl border p-5 sm:p-6"
          aria-labelledby="fixture-search-title"
        >
          <p className="text-accent text-xs font-semibold tracking-[0.18em] uppercase">
            Tu fixture
          </p>
          <h2 id="fixture-search-title" className="mt-2 text-2xl font-bold">
            ¿Cuándo juego?
          </h2>
          <p className="text-foreground/70 mt-2 text-sm">
            Buscá por tu nombre, apellido o pareja para ver todos tus partidos.
          </p>
          <label className="mt-5 block" htmlFor="fixture-search">
            ...
            <input
              ...
              className="border-border bg-muted/40 focus:border-accent focus:ring-accent/20 w-full rounded-xl border px-4 py-3 text-base transition outline-none focus:ring-2"
            />
          </label>
              ...
              {matchesForSearch.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-foreground/70 text-sm">
                    {matchesForSearch.length}{" "}
                    {matchesForSearch.length === 1
                      ? "partido encontrado"
                ...
                <p className="border-border text-foreground/70 rounded-xl border border-dashed p-4 text-sm">
      ...
      {categories.map(categoria => (
        <Category
          key={categoria}
          categoria={categoria}
          parejas={state.tournament.parejas.filter(
            pareja => pareja.categoria === categoria
          )}
          partidos={state.tournament.partidos.filter(
            partido => partido.categoria === categoria
          )}
          delayMinutes={visibleTournamentStatus?.delayMinutes}
        />
      ))}
